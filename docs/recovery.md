...
an extra screening step during recovery.

Private fee claims restore their exact saved funding record and beneficiary,
revalidate canonical bridge funding, and preserve the original message position.
Recovery does not repeat approval or deposit. Deployment retains its predicted
board identity and saved portal binding; conflicting existing state or a missing
saved portal remains unresolved. Moderator recovery restores the exact saved
method and arguments and checks current authority before preparing fees. A daemon
job must match that saved operation; a different job cannot replace it.
