...
;(function() {
  ...
  // Caller-independent public views use the SDK default sender (NO_FROM).
  // This avoids placing the private account in public RPC simulation inputs.
  // Private utility reads and signed operations retain their original owner.
  async function resolvePostId(a, contract, address, target) {
    ...
    if (target.postId !== undefined && target.postId !== null) {
      ...
      id = canonicalPostId(a, await contract.methods.get_post_id(BigInt(order)).simulate({ from: a.NO_FROM }));
    }
    const exists = unwrapPostValue(await contract.methods.get_post_exists(new a.Fr(BigInt(id))).simulate({ from: a.NO_FROM }));
  ...
  async function readPolicySnapshot(a, contract, address) {
    const snapshot = unwrapPostValue(await contract.methods.get_moderation_policy_snapshot().simulate({ from: a.NO_FROM }));
  ...
  g.BillboardPostCodec = Object.freeze({ parsePostOperation, safePostOrder, canonicalPostId, resolvePostId, packPostMessage, decodePostMessage, withFreshPostState, dummyStateCanRetry, postStateCanRetry });
  ...
  g.runBillboardUser = async function(env, config) {
    ...
    try {
      ...
      if (portalDeployed) {
        ...
        const expectedHex = '0x' + l2AddrHex.toLowerCase().replace(/^0x/, '').padStart(64, '0');
    ...
    if(ethereumActions.includes(action)) {
      ...
      if(action==='recover-eth') {
        ...
        let refundedSavedWithdrawal=false;
        if(success&&recovered.request?.expected?.kind==='withdraw'&&config.withdrawTxHash){
          const leaf=computeWithdrawMessageLeaf(a,ethers,l2Addr,portalAddr,l1Account,BigInt(recovered.request.expected.amount),version,BigInt(nodeInfo.l1ChainId));
          let witness;
          try{witness=await aztecNode.getL2ToL1MembershipWitness(a.TxHash.fromString(config.withdrawTxHash),leaf);}
          catch{throw Object.assign(new Error('Could not verify which withdrawal the refund completed.'),{code:'BB_RECOVERY_UNKNOWN'});}
          if(witness){
            const data=new ethers.Interface(PORTAL_ABI).encodeFunctionData('withdraw',[BigInt(witness.epochNumber),BigInt(witness.numCheckpointsInEpoch),BigInt(witness.leafIndex),witness.siblingPath.toBufferArray().map(buf=>'0x'+Buffer.from(buf).toString('hex'))]);
            refundedSavedWithdrawal=data.toLowerCase()===recovered.request.data.toLowerCase();
          }
        }
    ...
    if (l2NoteInfo && l2NoteInfo.amount > 0n) {
      ...
      try {
        ...
        let maxSaveUp = 16;
        if (contract) {
          const cdR = await contract.methods.get_base_cooldown().simulate({ from: a.NO_FROM });
          baseCooldown = BigInt(extractInt(cdR));
          const mdR = await contract.methods.get_min_deposit().simulate({ from: a.NO_FROM });
          minDepositL2 = BigInt(unwrapPostValue(mdR).toString());
          const msuR = await contract.methods.get_max_save_up().simulate({ from: a.NO_FROM });
    ...
    // ============================================================
    if (action === 'status') {
      return { ok: true, state: stateStatus, withdrawTxHash:config.withdrawTxHash||null, l2Addr: l2AddrHex, portalAddr, l1Account, portalL1Balance: portalL1Balance.toString(), l2Note: l2NoteInfo, handles: { pxe, wallet, contract, aztecNode, rawNode, address, l2Addr, contractSalt, version, nodeInfo, depositChainId: selectedChain, withdrawTxHash:config.withdrawTxHash||null } };
    }
    ...
    async function sendCensorPrivate(censorWallet, censorAddress, censorContract, kind, args) {
      ...
      if(resumedModeratorOperation) {
        ...
        const current=unwrapPostValue(await a.boundedTransactionRead(()=>censorContract.methods.get_censor().simulate({ from: a.NO_FROM }),20000));
    ...
    async function doPostAttempt(postNonce) {
      ...
      if (isDummy) {
        ...
        const infoResult = await readDepositInfo();
    ...
    async function doList() {
      ...
      const countResult = await contract.methods.get_post_count().simulate({ from: a.NO_FROM });
      const count = safePostOrder(countResult);
      if (!jsonOutput) log('  Post count: ' + count, 'info');

      // Check censor config
      const censorResult = await contract.methods.get_censor().simulate({ from: a.NO_FROM });
      let cv = censorResult;
      if (cv && cv.result !== undefined) cv = cv.result;
      if (cv && cv.value !== undefined) cv = cv.value;
      const censorAddr = cv && cv.toString ? cv.toString() : (cv ? '0x' + BigInt(cv).toString(16).padStart(64, '0') : null);
      const kResult = await contract.methods.get_k_multiplier().simulate({ from: a.NO_FROM });
      const kMult = extractInt(kResult);
      ...
      const cwResult = await contract.methods.get_censor_window().simulate({ from: a.NO_FROM });
      const censorWindow = Number(extractInt(cwResult));
      const msuResult = await contract.methods.get_max_save_up().simulate({ from: a.NO_FROM });
      const maxSaveUp = Number(extractInt(msuResult));
      ...
      for (let i = 0; i < count; i++) {
        const postId = await resolvePostId(a, contract, address, { postIndex: i });
        const idField = new a.Fr(BigInt(postId));
        const postResult = await contract.methods.get_post(idField).simulate({ from: a.NO_FROM });
        const vals = extractFieldArray(postResult);
        const length = await contract.methods.get_post_length(idField).simulate({ from: a.NO_FROM });
        const msg = decodePostMessage(vals, length);

        // Check if flagged
        let flagged = false;
        const flagResult = await contract.methods.is_post_flagged(idField).simulate({ from: a.NO_FROM });
        let fv = flagResult;
        ...
        if (flagged) {
          const respResult = await contract.methods.get_censor_response(idField).simulate({ from: a.NO_FROM });
          const respVals = extractFieldArray(respResult);
          const reasonLength = await contract.methods.get_censor_response_length(idField).simulate({ from: a.NO_FROM });
          censorResponse = decodeModerationReason(respVals, unwrapPostValue(reasonLength));
          const fbResult = await contract.methods.get_post_flagged_by(idField).simulate({ from: a.NO_FROM });
          let fbv = fbResult;
        ...
        const timeResult = await contract.methods.get_post_time(idField).simulate({ from: a.NO_FROM });
        const timestamp = Number(extractInt(timeResult));

        const postPolicyVersion = canonicalPostId(a, await contract.methods.get_post_policy_version(idField).simulate({ from: a.NO_FROM }));
        const flagDeadline = String(unwrapPostValue(await contract.methods.get_post_flag_deadline(idField).simulate({ from: a.NO_FROM })));
        if (jsonOutput) {
    ...
    async function getL2Timestamp(a, aztecNode) {
      ...
      if ((typeof ts !== 'bigint' && typeof ts !== 'number') || !Number.isSafeInteger(Number(ts)) || ts < 0) throw new Error('Could not get a valid L2 timestamp from block');
      return Number(ts);
    }
    ...
    async function doDeclareImmoral() {
      ...
      try {
        const flaggedByResult = await contract.methods.get_post_flagged_by(postIdField).simulate({ from: a.NO_FROM });
    ...
    async function doTransferCensor() {
      ...
      // Verify
      try {
        const newCensorResult = await contract.methods.get_censor().simulate({ from: a.NO_FROM });
    ...
    result.withdrawTxHash = config.withdrawTxHash || null;
