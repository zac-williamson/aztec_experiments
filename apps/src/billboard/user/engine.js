  ...
  g.runBillboardUser = async function(env, config) {
    ...
    if(action==='recover') {
      ...
      try { receipt = await transactionJournal.recover(); }
        ...
        // Only a real post has the independent on-chain identity needed for this
        // replacement route. Other operations retain their existing recovery path.
        let kind;try{kind=JSON.parse(saved.operation)?.kind;}catch{throw error;}
        if(kind!=='post')throw error;
        ...
        log('Saved post proof is stale. Restoring the same message and post identity for a fresh proof.', 'info');
      }
      if(receipt) {
    ...
    async function sendPrivate(kind, args) {
      if(kind!=='post'||args[4]===true) {
        ...
        transactionJournal.setOperation(JSON.stringify({kind:kind==='post'?'dummy':kind}));
      }
