...
export function createFileJournalStorage(directory) {
  ...
  function read(key) {
    ...
    try {const st=fs.fstatSync(fd);if(!st.isFile()||(st.mode&0o077)||st.size>32*1024*1024+1024)throw new Error('Invalid journal record');return fs.readFileSync(fd,'utf8');}finally {fs.closeSync(fd);}
  }
  return {async read(key){return read(key);},async compareAndSwap(key,previous,next){
    ...
    try {
      ...
      if(typeof next!=='string'||Buffer.byteLength(next)>32*1024*1024+1024)throw new Error('Invalid journal update');
