...
contract CallbackBridge {
    ...
    function configure(BillboardPortal target, bool callbacks, bool fail,
        address owner, uint128 amount, uint256 total) external {
        portal = target; attack = callbacks; reject = fail;
        watchedOwner = owner; expectedAmount = amount; expectedTotal = total;
    }

    function callback() private {
        calls++;
        uint128 amount = portal.getDeposit(watchedOwner);
        require(amount == expectedAmount, "Wrong receipt at dependency boundary");
...
contract PortalReentrancyTest is PortalFixtures {
    ...
    function setUp() public {
        ...
        configure(outbox, false, false, 0, 0);
    }
    function configure(CallbackBridge bridge, bool attack, bool reject, uint128 amount, uint256 total) private {
        bridge.configure(portal, attack, reject, ALICE, amount, total);
    }
    function activate() private { portal.activate(1, 1, 0, new bytes32[](0)); }
    function prepareDeposit() private {
        activate(); configure(inbox, false, false, 2 ether, 2 ether);
        depositAs(portal, ALICE, 2 ether);
    }
    function assertAccounting(uint128 amount, uint256 total, uint256 balance) private view {
        uint128 actualAmount = portal.getDeposit(ALICE);
        require(actualAmount == amount, "Wrong final receipt");
    ...
    function testOutboxActivationRejectsAllReentrantEntrypoints() public {
        configure(outbox, true, false, 0, 0); activate();
    ...
    function testInboxDepositRejectsAllReentrantEntrypointsAfterEffects() public {
        activate(); configure(inbox, true, false, 2 ether, 2 ether);
        depositAs(portal, ALICE, 2 ether);
        require(inbox.calls() == 1 && inbox.guardedCallbacks() == 3, "Missing deposit callbacks");
        assertAccounting(2 ether, 2 ether, 2 ether);
    ...
    function testRejectingOutboxRollsBackActivationAndCanRetry() public {
        configure(outbox, true, true, 0, 0);
    ...
    function testRejectingInboxRollsBackReceiptValueAndCanRetry() public {
        activate(); configure(inbox, true, true, 2 ether, 2 ether);
        vm.deal(ALICE, 2 ether);
        expectDependencyFailure(); vm.prank(ALICE); portal.deposit{value:2 ether}(bytes32(uint256(123)));
        assertAccounting(0, 0, 0);
        require(ALICE.balance == 2 ether, "Rejected deposit retained value");
        require(inbox.calls() == 0 && inbox.guardedCallbacks() == 0, "Rejected Inbox effects retained");
        configure(inbox, true, false, 2 ether, 2 ether); depositAs(portal, ALICE, 2 ether);
        assertAccounting(2 ether, 2 ether, 2 ether);
    }
    function testRejectingOutboxRollsBackWithdrawalAndCanRetry() public {
        prepareDeposit(); configure(outbox, true, true, 0, 0);
        expectDependencyFailure(); withdrawAs(portal, ALICE);
        assertAccounting(2 ether, 2 ether, 2 ether);
        require(ALICE.balance == 0, "Rejected withdrawal changed balance");
        require(outbox.calls() == 1 && outbox.guardedCallbacks() == 0, "Rejected consume retained effects");
        configure(outbox, true, false, 0, 0); withdrawAs(portal, ALICE);
        assertAccounting(0, 0, 0);
