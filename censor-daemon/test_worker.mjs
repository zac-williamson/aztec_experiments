...
function setup(t){
 ...
 f.data={scope,checkpoint:{number:10,hash:hex(100)},policies:[{policyVersion:hex(10),text:'Historical policy A',censorWindow:'1000'},{policyVersion:hex(11),text:'Current policy B',censorWindow:'1000'}],posts:[f.post()]};
...
test('evaluation uses historical policy A even when current policy is B',async t=>{const f=setup(t);f.evaluate=async(text,policy)=>{assert.equal(policy,'Historical policy A');assert.equal(text,f.data.posts[0].text);return {isViolation:false};};await f.run();assert.equal(f.record().job.state,'evaluated-ok');});
...
test('dropped transaction after deadline never signs again',async t=>{const f=setup(t);f.violation();await f.run();f.time(2001000);f.node.getTxReceipt=async()=>({status:'dropped',txHash:hex(50)});await f.run();assert.equal(f.calls.submit,1);assert.equal(f.record().job.state,'manual-review');});
...
test('deadline passing during inference prevents any signing',async t=>{const f=setup(t);f.data.posts[0].timestamp='1';f.data.posts[0].flagDeadline='1001';f.evaluate=async()=>{f.time(1001000);return {isViolation:true,reason:'Spam'};};await f.run();assert.equal(f.calls.submit,0);assert.equal(f.record().job.state,'expired');});
...
test('lost response with empty journal after deadline remains explicit manual review',async t=>{const f=setup(t);f.violation();f.signer.submitFlag=()=>{f.calls.submit++;throw Error('lost');};await f.run();f.signer.inspectFlag=()=>{f.calls.inspect++;return {txHash:null};};f.time(2001000);await f.run();assert.equal(f.record().job.state,'manual-review');assert.equal(f.calls.submit,1);assert.equal(f.calls.inspect,1);});
