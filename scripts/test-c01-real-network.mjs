...
async function worker(directory) {
  ...
  let anvilClosed;
  const output={passed:false,profile:'direct real-verifier local deployment; no board or epoch proof'};
  ...
  try {
    ...
    const [{getGenesisValues},{getConfigEnvVars},{deployC01RealProtocol}]=await Promise.all([
      import('@aztec/world-state/testing'),import('@aztec/aztec-node/config'),import('./c01-real-deployment.mjs')]);
    let preparation;
    if(process.env.C01_BOARD_PROOF==='true'){const {prepareC01BoardFlow}=await import('./c01-board-flow.mjs');preparation=await prepareC01BoardFlow({bbBinaryPath:path.join(directory,'bb-one-thread'),directory});}
    ...
    mark('deploy-real-verifier');
    const result=await deployC01RealProtocol({rpcUrl,privateKey:identity.privateKey,config,genesisArchiveRoot,fundingNeeded});
    ...
    if(process.env.C01_START_NODE==='true'){
      mark('start-real-node-and-prover');
...
async function parent() {
  ...
  const id = randomUUID();
  const evidence = path.join(ROOT, 'execution/evidence/C01', `real-network-${id}.json`);
  ...
  try {
    ...
    if(settle){
      ...
      assert.equal(epochManifest.bbSha256,'208cc0d9046603f31a8dc6c5ed0de529ccc63155a22078d409262ec6e4122031');
      const entry=epochManifest.compressed;assert.equal(entry.name,'bn254_g1_compressed.dat');assert.equal(entry.bytes,541065216);assert.equal(entry.chunkBytes,4194304);assert.equal(entry.chunkSha256.length,129);
      const binary=await fs.readFile(bb);assert(binary.indexOf(Buffer.concat(entry.chunkSha256.map(hex=>Buffer.from(hex,'hex'))))>=0);
      const bytes=await fs.readFile(path.join(source,entry.name));assert.equal(bytes.length,entry.bytes);assert.equal(sha(bytes),entry.sha256);
      for(let i=0;i<129;i++)assert.equal(sha(bytes.subarray(i*entry.chunkBytes,(i+1)*entry.chunkBytes)),entry.chunkSha256[i]);
      await fs.writeFile(path.join(crs,entry.name),bytes,{flag:'wx',mode:0o400});report.epochSetup=epochManifest;
      for(const name of ['bn254_g1.dat','grumpkin_g1_v2.flat.dat'])await fs.chmod(path.join(crs,name),0o600);
    }
    ...
    child.stderr.on('data',chunk=>{
      ...
      if(settle && stderrBuffer.includes('AVM is not supported in this build.')) stop('unsupported-avm-binary');
    });
    ...
    for(const entry of report.setup){
      ...
      if(settle&&['bn254_g1.dat','grumpkin_g1_v2.flat.dat'].includes(entry.name)){
        assert(bytes.length>=entry.bytes&&bytes.length%64===0&&bytes.length<=1082130432);
        assert.equal(sha(bytes.subarray(0,entry.bytes)),entry.sha256);
        report.derivedSetup.push({name:entry.name,bytes:bytes.length,sha256:sha(bytes),verifiedOriginalPrefixBytes:entry.bytes});
