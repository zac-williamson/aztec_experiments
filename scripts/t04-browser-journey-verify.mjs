...
export function journeyExitLeaf({scope,depositor,amount}){
 assert(BigInt(amount)>0n);
 const content=sha256ToField([Buffer.from(encodeEscrowCommitment('exit',scope,{depositor,amount:String(amount)}))]);
...
export async function verifyJourneyRefund({l1Client,portalAbi,portalAddress,depositor,amount,txHash,before}){
 ...
 assert.deepEqual(await read('getDeposit'),0n);
 ...
 return {passed:true,txHash,amount:String(amount),gasWei:String(gas)};
}
...
export async function verifyJourneyPrivateChain({wallet,node,instance,artifact,account,privateFee,transactions,message,receipt,before}){
 ...
 assert.equal(fields[0],1n);assert.equal(fields[2],receipt.amount);assert.equal(fields[3],BigInt(receipt.depositor));assert.equal(fields[6],0n);
 ...
 assert.deepEqual((await query('get_deposit_info',account.address,chain)).map(n),Array(10).fill(0n));
