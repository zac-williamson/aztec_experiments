...
if(script==='scripts/test-c01-application.mjs'&&(args.length!==1||!['--browser-post','--browser-journey'].includes(args[0])))throw Error('Only fixed browser-post/browser-journey modes are supported by this aggregate supervisor');
const reportPath=path.resolve(ROOT,output??'');
const evidenceDirectory=script==='scripts/test-c01-application.mjs'&&args[0]==='--browser-journey'?'T04':'U01';
