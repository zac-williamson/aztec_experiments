...
const c=vm.createContext({setTimeout,clearTimeout,console});vm.runInContext(source,c);
...
test('confirmed Ethereum phase survives pending claim and retry only uses original claim hash',async()=>{
 ...
 const ctx=vm.createContext({_stateResult:{state:'zero_balance_need_deposit'},document:{getElementById:id=>elements[id]},clearMissingHighlight(){},highlightMissing(){},_getConfigRevision:()=>1,_commonConfig:()=>({}),publicOperationFailure:e=>e,log(){},nextPage(){},showPage(){},callEngine:async(action,_status,config)=>{calls.push({action,config});if(action==='deposit')return{depositInfo:{txHash}};throw Object.assign(Error('pending'),{code:'BB_DEPOSIT_MESSAGE_PENDING'});}});
...
test('actual navigation failure preserves action-updated claim label and restores ordinary labels',async()=>{
 ...
 for(const phaseChanged of [true,false]){
  ...
  const nav=vm.createContext({_pages:[{busyText:'Processing...',statusId:'status',action:async()=>{if(phaseChanged)button.textContent='Claim deposit →';throw Error('pending');}}],_currentPage:0,document:{getElementById:()=>button},clearStatus(){},log(){},console:{error(){caught();}},setTimeout,nextPage(){throw Error('Must not advance after failure');}});
