...
async function browserWorker(directory) {
  ...
  try {const control=await readPrivateControl();const {validateBrowserWorkerControl}=await import('./t04-browser-journey.mjs');validateBrowserWorkerControl(control,{directory});const {runU01BrowserPost}=await import('./u01-browser-post.mjs');const journeyDriver=control.browserJourney?(await import('./t04-browser-journey.mjs')).driveT04BrowserJourney:undefined;result=await runU01BrowserPost({...control,journeyDriver,directory,diagnostic:control.browserJourney,observeProofStages:!control.browserJourney&&!control.browserRecovery,onStage:stage=>process.stdout.write(JSON.stringify({browserStage:stage})+'\n')});}
...
async function parent() {
  ...
  assert(!(browserJourney&&browserRecovery));
  const browserPost=process.argv[2]==='--browser-post'||browserJourney||browserRecovery;
  ...
  try {
    ...
    if(browserPost){
      ...
      await new Promise((resolve,reject)=>{reservation.once('error',reject);reservation.listen(0,'127.0.0.1',resolve);});
      const port=reservation.address().port;await new Promise(resolve=>reservation.close(resolve));
      browserControl={browserJourney,browserRecovery,origin:'https://127.0.0.1:'+port,rpcToken:randomBytes(32).toString('hex'),backupPassword:randomBytes(32).toString('base64url')};
    ...
    report.exit = await awaitCoordinatorAndBrowser({coordinator:Promise.race([finished, new Promise(resolve => {
      outerTimer = setTimeout(() => resolve({ supervisionTimeout: true }), DEADLINE_MS + 10000);
    })]),getBrowser:()=>browserPromise,stop});
