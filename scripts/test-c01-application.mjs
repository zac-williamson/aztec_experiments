...
async function parent() {
  ...
  assert(process.argv.length===2||(process.argv.length===3&&['--node','--board-proof','--include','--ready','--settle','--bridge','--screening','--contention','--posting-diagnostic','--sponsorship','--sponsored-post'].includes(process.argv[2])),'Unsupported harness arguments');
  ...
  const id = randomUUID();
  const evidence = path.join(ROOT, sponsorship?'execution/evidence/W01':contention?'execution/evidence/C03':screening?'execution/evidence/C02':'execution/evidence/C01', `application-${id}.json`);
  ...
  try {
    ...
    report.networkProofs=false;report.controlledSettlement=true;report.binarySha256=binarySha;
    assert(!bb.includes("'"));
    ...
    report.sponsoredPosting=sponsoredPosting;report.sponsorship=sponsorship;report.startNode=startNode;report.boardProof=boardProof;report.settle=settle;report.bridge=bridge;report.screening=screening;report.contention=contention;report.postingDiagnostic=postingDiagnostic;report.expectedAuthorCount=contention&&!postingDiagnostic?10:1;
