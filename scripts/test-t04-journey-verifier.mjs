...
test('exit helper binds receipt and domain inputs and rejects malformed/out-of-range shapes',()=>{
 ...
 for(const changed of [{...input,amount:'1000000000000001'},{...input,scope:{...input.scope,l1ChainId:'31338'}},{...input,scope:{...input.scope,rollupVersion:'2'}}])assert(!journeyExitLeaf(changed).leaf.equals(baseline));
...
test('actual refund verifier decodes a real ABI event and rejects wrong receipt identity/accounting',async()=>{
 const declaration='event Withdrawn(address indexed depositor,uint128 amount)';
 const portalAbi=parseAbi([declaration,'function getDeposit(address) view returns (uint128)','function totalDeposited() view returns (uint256)']);
 const iface=new Interface([declaration]),portalAddress=input.scope.portalAddress,depositor=input.depositor;
 const txHash='0x'+'ab'.repeat(32),blockHash='0x'+'cd'.repeat(32),amount=BigInt(input.amount);
