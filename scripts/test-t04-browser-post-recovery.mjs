...
async function fixture(schema=1,action='post'){
...
test('withdrawal recovers once then reconnects Ethereum to display pending refund without submitting it',async()=>{
 const f=await fixture(1,'withdraw');try{
  ...
  assert.deepEqual(f.calls,['open-withdraw','withdraw-page','withdraw','restart','#wbAccountMenu > summary','#wbPassword','restore','#wbAccountMenu > summary','recover','wallet-setup','#page-0','#wbEthBrowserBtn','wallet-choice','#page-4']);
 }finally{await f.cleanup();}
});
test('unknown recovery action is rejected before interacting with the page',async()=>{
 const f=await fixture();try{await assert.rejects(runT04BrowserPostRecovery({...f.args,action:'refund'}));assert.deepEqual(f.calls,[]);}finally{await f.cleanup();}
});
