// Synthetic persisted history: exercises pinned PXE storage and oracle selection,
// not note commitment validity or 1,000 proved application transactions.
...
// Internal APIs intentionally pinned by package-lock.json. Fail visibly if PXE changes.
const pxeBase = new URL('../', import.meta.resolve('@aztec/pxe/config'));
const { NoteStore } = await import(new URL('storage/note_store/note_store.js', pxeBase));
const { NoteService } = await import(new URL('notes/note_service.js', pxeBase));
const { pickNotes } = await import(new URL('contract_function_simulator/pick_notes.js', pxeBase));
let dir;
...
async function open() {
  store = await AztecLMDBStoreV2.new(dir, 16 * 1024, 4);
...
try {
  dir = await mkdtemp(join(tmpdir(), 'billboard-c04-note-store-'));
  await open();
  ...
  assert.equal((await query(chain, target)).length, 2);
  console.log(JSON.stringify({ status: 'passed', fixture: 'synthetic persisted NoteDao history; no proofs',
