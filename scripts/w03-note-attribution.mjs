...
export async function qualifyDummyNoteAttribution({ wallet, board, owner, depositChainId, originalNote, node, privateFeeAction, discardUnsubmittedFee }) {
  ...
  assert.equal(typeof discardUnsubmittedFee, 'function', 'Unsubmitted dummy fee accounting callback required');
  const args = [depositChainId, Fr.ZERO, Array.from({length:32}, () => Fr.ZERO), 0, true, undefined, undefined];
  const started = performance.now();
  let prepared = await proveApplicationAction({payerMode:'private',wallet, owner, interaction:board.methods.post(...args),
    privateFeeAction:context => privateFeeAction({...context,kind:'post',args})});
  ...
  discardUnsubmittedFee(prepared.maximumFee);
