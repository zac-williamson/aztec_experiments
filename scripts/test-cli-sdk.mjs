...
async function child(lane) {
  ...
  if (lane === 'user') {
    ...
    const vectors = JSON.parse(fs.readFileSync(path.join(ROOT, 'scripts/fixtures/protocol/commitments-v1.json'), 'utf8'));
...
if (process.argv[2] === '--child') {
  ...
  console.log(JSON.stringify({
    ...
    sourceHashes: Object.fromEntries([...new Set(Object.values(sources)), 'shared/aztec-lib.js', 'shared/cli-simulator.mjs', 'apps/src/billboard/user/engine.js', 'scripts/fixtures/protocol/commitments-v1.json'].map(name => [name, sha(fs.readFileSync(path.join(ROOT, name)))])),
    results,
  }, null, 2));
