...
test('actual navigation gate rejects unpaid debt without polling or signing',async()=>{
 let reads=0;const ctx={application:{connected:true,revision:0},_getConfigRevision:()=>1,readCurrentDeposit:async()=>{reads++;return {...deposit,nextAllowedTime:11n,chainTime:10};},getL2Timestamp:async()=>10,withdrawalReadiness:ready,log(){}};
...
test('actual navigation gate refuses a stale configuration response',async()=>{
 let revision=1;const ctx={application:{connected:true,revision:0},_getConfigRevision:()=>revision,readCurrentDeposit:async()=>{revision++;return deposit;},getL2Timestamp:async()=>10,withdrawalReadiness:ready,log(){}};
...
for(const role of ['user','censor']){
 ...
 function fixture(stale=false){const feed=new Element('div'),meta=new Element('div');let revision=1;const payload='<img src=x onerror=alert(1)>';
  ...
  ctx.application={readFeed:()=>ctx.window.BillboardPublic.readFeed({expectedConfig:ctx._getPublicConfig()})};return{feed,meta,ctx,payload};}
