...
for(const role of ['user','censor']){
 ...
 test(role+' feed uses native details and literal untrusted text',async()=>{const f=fixture();await vm.runInNewContext(code+';refreshBillboard()',f.ctx);const detail=f.feed.children[0].children[1];assert.equal(detail.tag,'details');assert.equal(detail.children[0].tag,'summary');assert.equal(detail.children[1].textContent,f.payload);assert.equal(detail.children[2].textContent,'Moderator reason: '+f.payload);});
