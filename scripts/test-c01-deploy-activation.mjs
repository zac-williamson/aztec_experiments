...
const included=(extra={})=>({status:TxStatus.CHECKPOINTED,executionResult:TxExecutionResult.SUCCESS,blockNumber:5,blockHash:blockA,...extra});
...
function harness({receipts=[included()],tips=[5],blocks=[blockA],witness=true,activationStatus=1,enabled=true}={}){
  const context=vm.createContext({setTimeout});vm.runInContext(source,context,{filename:'deploy/engine.js'});
...
test('actual deployment helper waits for finality then forwards exact authenticated witness',async()=>{
  const f=harness({tips:[4,5,5]});await f.run();
  assert.equal(f.calls.wait,1);assert.equal(f.calls.receipt.length,3);
...
test('unfinalized receipt never requests a witness or activates',async()=>{
  const f=harness({tips:[4]});await assert.rejects(f.run(),/not finalized/);
...
test('missing witness never activates a finalized transaction',async()=>{
  const f=harness({witness:false});await assert.rejects(f.run(),/not finalized/);
  assert.equal(f.calls.witness.length,3);assert.equal(f.calls.activate.length,0);
});
for(const [name,receipt,message] of [
  ...
  ['dropped',{status:TxStatus.DROPPED},/dropped/],
...
test('pending and proposed receipts do not qualify even with a high finalized tip',async()=>{
  ...
  await assert.rejects(f.run(),/not finalized/);assert.equal(f.calls.witness.length,0);assert.equal(f.calls.activate.length,0);
});
test('receipt block must still match canonical block at its height',async()=>{
  const f=harness({blocks:[blockB]});await assert.rejects(f.run(),/not finalized/);
  assert.equal(f.calls.witness.length,0);assert.equal(f.calls.activate.length,0);
});
test('re-inclusion during witness resolution refreshes receipt and waits for the new block',async()=>{
  ...
  await f.run();assert.equal(f.calls.wait,1);assert.equal(f.calls.receipt.length,4);
...
test('drop during witness resolution prevents activation',async()=>{
  const f=harness({receipts:[included(),{status:TxStatus.DROPPED}]});await assert.rejects(f.run(),/dropped/);
