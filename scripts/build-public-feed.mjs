...
import {CONTRACT_INSTANCE_PUBLISHED_EVENT_TAG,ProtocolContractAddress} from '@aztec/protocol-contracts';
...
export async function buildPublicFeed(){
 ...
 const iface=new Interface(['function L2_CONTRACT() view returns(bytes32)','function ROLLUP() view returns(address)','function VERSION() view returns(uint256)','function L1_CHAIN_ID() view returns(uint256)','function depositsEnabled() view returns(bool)']);
 const classId=(await getContractClassFromArtifact(loadContractArtifact(artifact))).id.toString();
 const metadata={schemaVersion:1,classId,instancePublicationTag:CONTRACT_INSTANCE_PUBLISHED_EVENT_TAG.toString(),registrationNullifier:(await siloNullifier(ProtocolContractAddress.ContractClassRegistry,Fr.fromString(classId))).toString(),artifact:{outputs:{structs:{events:Object.values(definitions).map(x=>x.abiType)}}},eventTags:Object.fromEntries(Object.entries(definitions).map(([k,v])=>[k,v.tag])),storage:{portal:slot('portal_address'),config:slot('config')},portalSelectors:Object.fromEntries(['L2_CONTRACT','ROLLUP','VERSION','L1_CHAIN_ID','depositsEnabled'].map(n=>[n,iface.encodeFunctionData(n)]))};
