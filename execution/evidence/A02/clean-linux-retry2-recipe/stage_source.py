...
target = root / '.build/A02-clean-linux-retry2-source'
if target.exists():
...
script = root / '.build/A02-linux-retry2-tools/run.sh'
...
metadata = {
    ...
    'helperHashes': {name: hashlib.sha256((root / '.build/A02-linux-retry2-tools' / name).read_bytes()).hexdigest() for name in ['run.sh','steps.py','stage_source.py','pack.py','launch.py']},
...
(root / 'execution/evidence/A02/clean-linux-retry2-inputs.json').write_text(json.dumps(metadata, indent=2) + '\n')
(target / 'clean-source-inventory.json').write_text(json.dumps({'sourceInputs': inputs, 'sourceSymlinks': symlinks}, indent=2) + '\n')
shutil.copy2(root / '.build/A02-linux-retry2-tools/steps.py', target / 'clean-steps.py')
