    ...
    with (out/(label+'.log')).open('w') as log:
        ...
        def group_states():
            ...
            for stat in pathlib.Path('/proc').glob('[0-9]*/stat'):
                try:
                    raw=stat.read_text(); tail=raw[raw.rfind(')')+2:].split()
                    if int(tail[2])==child.pid:
                        states.append({'pid':int(stat.parent.name),'state':tail[0],'ppid':int(tail[1]),'pgrp':int(tail[2])})
                except FileNotFoundError: pass
            return sorted(states,key=lambda item:item['pid'])
        entry['processGroupBeforeCleanup']=group_states()
        actions=[]
        def group_alive():
        ...
        child.poll()
        entry['processGroupAfterCleanup']=group_states()
