...
The wallet's existing private PXE store supplies history and supports reload. Its keys remain local and never enter the public feed. Pinned PXE internally scans owner/slot candidates before applying field selectors: the oracle response is bounded, while overall storage/query work is not claimed constant. C04 measures actual persistence and lookup beyond 16, 32 and 1000 entries, multiple deposits and missing/duplicate/stale behavior. Recovery and broader performance qualification remain explicit subsequent work; no arbitrary lifetime-history cutoff is imposed by this utility.
