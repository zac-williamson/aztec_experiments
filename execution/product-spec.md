...
| Load/history | Ten distinct authors prepare from one anchor; all make progress. History capacity beyond 1,000 records across three deposit cycles, using the recorded C04 layered method: stateful boundary lifecycles, seeded authenticated continuation/exit, actual persisted PXE/browser storage, and separate genuine-proof integration. Synthetic records are not executed publications. No new contract-imposed cutoff or shared-counter invalidation. Each legitimate attempt either succeeds or receives an actionable recoverable outcome; no unexplained starvation over ten consecutive blocks after eligibility. |
| Browser reference | Chrome desktop is the supported browser for the current milestone (user direction, 2026-09-21); record the exact version. Firefox, WebKit and Safari qualification is deferred. Reference hardware: Apple Silicon with at least 16 GB RAM, and a comparable Linux/Windows Chromium host if available; record actual CPU/RAM. Cold proving p95 <=180 s and warm proving p95 <=90 s over >=30 posts per supported engine, excluding documented chain inclusion/finality waits. Initial public-feed p95 <=3 s over 30 fresh loads at 20 Mbps/100 ms RTT; no private wallet/proving download required for reading. |
