...
if(!['read','write','operations'].includes(scenario))throw Error('Choose read, write or operations qualification explicitly');
...
try{
 await supervisor.start('plugin-browser',process.execPath,['--env-file=plugins/.env','plugins/devnet/supervised-worker.mjs','--test-wallet',...(scenario==='write'?['--github-writes']:[]),...(scenario==='operations'?['--operations']:[])],{cwd:process.cwd(),env:{...process.env,PLUGIN_SUPERVISED:'true'},onRecord:r=>{if(r.progress?.startsWith('BROWSER_DIRECTORY '))report.runDirectory=r.progress.slice('BROWSER_DIRECTORY '.length);if(r.progress?.startsWith('RESULT_PATH '))report.resultPath=r.progress.slice('RESULT_PATH '.length);console.log(r.progress);}});
}finally{
