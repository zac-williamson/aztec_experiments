...
## Active replacement: per-plugin Aztec escrow

User accepted separate balances per plugin. Implement `ESCROW_SPEC.md`; supersedes
shared billing and Ethereum pay-per-post proposals. Root is sole writer. Checkpoint:
examined current board hook, posting coordinator and bridge APIs; no implementation
qualified yet. Build contracts, provider metering and browser flow, then run full
fund/post/settle/withdraw acceptance. Previous fixed-fee evidence is historical only.

Replacement checkpoint (2026-09-22): implemented the per-plugin escrow, USDC
portal, generic board hook, model metering, account client and browser flow.
13 escrow Noir tests pass. 238 affected JavaScript/wallet/journal/interface tests
pass. The maintained Solidity regression profile passes 40 tests. Contract and
browser builds pass. These are intermediate results, not end-to-end qualification.

Browser attempt sYFS3R stopped before funding because the pinned SDK requires
AztecAddress.fromStringUnsafe; fixed both client and service constructors.
Attempt YJU86J approved/deposited real local USDC through MetaMask but failed at
Aztec claim. Its harness omitted the plugin status from error diagnostics, so the
exact simulation error was not retained. Inspection found missing integration
with the actual author wallet's journal/submission callback, and empty checkpoint
production left enabled before claim; both are corrected. SccCqm is the next full
run, including an unfunded composer post that must roll back with no provider cost.
