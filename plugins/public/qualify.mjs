...
const [requestedPhase,directory,...args]=process.argv.slice(2),phase=requestedPhase==='recover-fund'?'fund':requestedPhase;if(!['fund','recover-fund','post','reply','redeem'].includes(requestedPhase)||args.length!==2)throw Error('Provide phase, author directory, service config and site config');
const report={phase,requestedPhase},supervisor=new Supervisor({report});
try{await supervisor.start('public-wallet',process.execPath,['--env-file=plugins/.env','plugins/public/worker.mjs',requestedPhase,directory,...args],{cwd:process.cwd(),env:process.env,onRecord:r=>console.log(r.progress)});}finally{
