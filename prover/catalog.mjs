...
export async function loadCircuitCatalog(){
 ...
 const artifacts=[SchnorrInitializerlessAccountContractArtifact,...Object.values(ProtocolContractArtifact)];
 ...
 const raw=JSON.parse(await fs.readFile(new URL('../apps/src/billboard/billboard_artifact.json',import.meta.url),'utf8'));for(const circuit of circuits.values())if(circuit.functionName.startsWith('Billboard:'))circuit.abi=raw.functions.find(f=>f.name===circuit.functionName.split(':')[1]).abi;
 return circuits;
}
