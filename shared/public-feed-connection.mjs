...
const incompatible=message=>Object.assign(Error(message),{code:'BB_BOARD_INCOMPATIBLE'});
...
export async function connectPublicFeed({nodeUrl,ethereumUrl,portalAddress,metadata,storage,fetchImpl,expectedConfig}){
  ...
  const call=async name=>{
    let value;try{value=await eth('eth_call',[{to:portalAddress,data:metadata.portalSelectors[name]},'latest']);}
    catch(error){if(error.code==='BB_PUBLIC_CALL_REVERTED')throw incompatible('Portal does not support this application');throw error;}
    if(typeof value==='string'&&/^0x[0-9a-fA-F]*$/.test(value)&&value.length!==66)throw incompatible('Portal does not support this application');
    return word(value);
  };
  ...
  if(typeof chain!=='string'||!/^0x[0-9a-fA-F]+$/.test(chain))throw Error('Invalid Ethereum chain response.');
  if(BigInt(chain)!==l1ChainId||rollup<=0n||rollup>=1n<<160n)throw incompatible('Portal network does not match this board.');
