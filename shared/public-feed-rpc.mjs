...
export function publicRpc(url,{fetchImpl=globalThis.fetch,timeoutMs=20000,maxBytes=4*1024*1024}={}) {
  ...
  return async(method,params=[])=>{
    if(!['node_findLeavesIndexes','node_getNodeInfo','node_getTxReceipt','node_getContract','node_getBlockData','node_getPublicLogsByTags','node_getPrivateLogsByTags','node_getPublicStorageAt','eth_chainId','eth_call'].includes(method))throw Error('Unsupported public RPC method.');
    ...
    try {
      ...
      if(method==='eth_call'&&result.jsonrpc==='2.0'&&result.id===requestId&&result.error?.code===3)throw Object.assign(Error('Contract call reverted'),{code:'BB_PUBLIC_CALL_REVERTED'});
      if(result.jsonrpc!=='2.0'||result.id!==requestId||result.error||!Object.hasOwn(result,'result'))throw Error();return result.result;
    } catch(error) {if(error.code==='BB_PUBLIC_CALL_REVERTED')throw error;throw Object.assign(new Error('Public RPC request failed. Cached data was preserved.'),{code:'BB_PUBLIC_FEED_UNAVAILABLE'});}
    finally {clearTimeout(timer);controller.abort();}
  };
}
export function publicNode(url,options){const rpc=publicRpc(url,options);return Object.freeze({
  findLeavesIndexes:(block,tree,leaves)=>rpc('node_findLeavesIndexes',[block,tree,leaves]),
  ...
  getPrivateLogsByTags:query=>rpc('node_getPrivateLogsByTags',[query]),
